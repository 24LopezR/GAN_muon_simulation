    ...
    for i in range(n_epochs):
        ...
        for j in range(bat_per_epo):
            for k in range(n_discrim_updates):
                ...
                d_loss1, acc_real = d_model.train_on_batch([X_real, in_data_real], y_real)
				...
				# update discriminator model weights
                d_loss2, acc_fake = d_model.train_on_batch([X_fake, in_data_fake], y_fake)
            for k in range(n_gen_updates):
                ...
                g_loss = gan_model.train_on_batch([z_input, in_data], y_gan)
            if (j+1) % 20 == 0:
