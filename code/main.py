...
if __name__ == "__main__":
	...
	print(' Training mode: {:<50}'.format(str(options.train)))
	...
	# Evaluate the model
	print('Scaler mean: {:<15}'.format(str(scaler.mean_)))
	print('Scaler variance: {:<15}'.format(str(scaler.var_)))
	real = data[:,4:]
	fake = generate_evaluation_samples(g_model, dataset, latent_dim, scaler)
	evaluate(real, fake)
	print_plots(real, fake, output)
	# _________________________________________________________________________________________
