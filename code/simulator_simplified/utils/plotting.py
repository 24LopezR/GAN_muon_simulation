...
def plot_hits(hits_real, hits_gen):
    ...
    ax1.hist(hits_real, density = False, color = 'black', range=(0,20), bins=20, label='Real')
    ax2.hist(hits_gen,  density = False, color = 'red',   range=(0,20), bins=20, label='Generated')
    ax1.legend()
    ax2.legend()
...
def print_plots(activations, activations_gen):
    plot_hits(activations, activations_gen)
