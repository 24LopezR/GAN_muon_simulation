...
EVAL_DATA_FILE = EVALUATION_SAMPLES_PATH + f"/evaluationSamples_seed20_{NSAMPLES}.csv"
...
"""
def load(inputfile):
    data_all = pd.read_csv(inputfile,header=None).to_numpy()
...
if __name__ == "__main__":
    ...
    z_noise = np.random.normal(size=(NSAMPLES, LATENT_DIM))
    fake_samples = g_model.predict([z_noise, np.hstack([dataset[:NSAMPLES,0:4], dataset[:NSAMPLES,8:]])], batch_size=NSAMPLES)
    end_time = time.time()
    print('    Number of gen samples = '+str(fake_samples.shape[0]))
    print('    Start time = '+str(start_time))
    print('    End time = '+str(end_time))
    print("--- Generation time: %s seconds ---" % (end_time - start_time))

    g4_dataset = SCALER.inverse_transform(dataset[:NSAMPLES,0:8])
    gan_dataset = SCALER.inverse_transform(np.hstack([dataset[:NSAMPLES,0:4], fake_samples]))
