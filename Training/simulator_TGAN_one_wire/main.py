...
if __name__ == "__main__":
    ...
    parser.add_option("-i", "--input", dest="input", type="string",
                      default='/home/ruben/Documents/TFM/GAN_muon_simulation/data/file_2.csv',
                      help="Input root filename")
    ...
    if options.train is not False:
        ...
        model = GAN(input_dim=input_dim, latent_dim=latent_dim, learning_rate=learning_rate, generator=g_model)
        # create the models
        d_model, g_model, gan_model = model.define_gan()
        d_model.summary()
        g_model.summary()
        gan_model.summary()
        # train the model
        g_model = train(g_model, d_model, gan_model, dataset, latent_dim, model.step_decay, n_discrim_updates,
                        n_gen_updates, epochs, nbatch)
    # _________________________________________________________________________________________
